---
brief_id: duet-20260605-094046
generated_at: 2026-06-05T09:40:46.075634+00:00
loop_id: ITER-010
mode: unattended
---
...
## Planner
- Candidate: gap-screenshot-structured-extraction
- Goal: Replace screenshot placeholder flow with structured extraction handoff
- Output: Create a local structured extraction artifact for screenshot uploads instead of returning only a placeholder payload.
- Output: Preserve raw image evidence and mark uncertain fields explicitly instead of guessing LOS or conclusions.
- Output: Keep the extraction handoff traceable so a later agent step can complete record-mistake safely.

## Critic
- Acceptance: Screenshot upload returns a durable extraction draft reference, not only a generic saved status.
- Acceptance: Draft output stores evidence path plus clearly empty or uncertain fields when the image cannot support them.
- Acceptance: Tests cover safe filename handling and the structured handoff contract.
- Safety: Do not hallucinate LOS, source, choices, or correct answers from unclear screenshots.
- Safety: Do not delete or rewrite the original screenshot evidence asset.
- Safety: Do not claim full AI extraction unless the workflow persists an explicit draft artifact.

## Synthesis
- Proceed with bounded task `gap-screenshot-structured-extraction`.
