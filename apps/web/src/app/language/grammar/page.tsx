...
export default function LanguageGrammar() {
  ...
  useEffect(() => {
    languageApi.segments()
      .then(({ segments: rows }) => {
        setSegments(rows);
        setSegmentId(rows[0]?.segment_id || '');
        if (rows.length === 0) setMessage('Import a sentence first.');
      })
      .catch(() => setMessage('Could not load corpus segments.'))
    ...
    <LanguageShell title="Make sentence structure visible." eyebrow="Grammar lens">
      <section className="motion-reveal card space-y-4">
        <div className="flex flex-col gap-3 sm:flex-row">
          ...
          <button type="button" disabled={loading || !segmentId} onClick={analyze} className="rounded-lg bg-accent-solid px-4 py-2 text-sm text-white disabled:cursor-not-allowed disabled:opacity-50">Analyze sentence</button>
        </div>
