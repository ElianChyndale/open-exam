---
title: "M04 — Introduction to the Global Investment Performance Standards (GIPS)"
description: "CFA Level I 2026 official module: Introduction to the Global Investment Performance Standards (GIPS)"
module: M04
subject: "Ethical and Professional Standards"
topic_area: Ethical_and_Professional_Standards
curriculum_year: 2026
official_module: "Module 4: Introduction to the Global Investment Performance Standards (GIPS)"
official_source: CFA Institute Learning Ecosystem scrape, generated 2026-05-25
note_type: official_module_projection
status: active
tags:
  - CFA_L1
  - Ethical_and_Professional_Standards
  - official_2026
---

# M04: Introduction to the Global Investment Performance Standards (GIPS)

> This file is aligned to the CFA Institute 2026 Level I module name and order. Legacy local notes were migrated below when a reliable match was found.

## Official Module Structure

- Learning Outcomes: Introduction to the Global Investment Performance Standards (GIPS)
- 4.01 | Introduction
- 4.02 | Why Were the GIPS Standards Created, Who Can Claim Compliance, & Who
- 4.03 | Composites
- 4.04 | Fundamentals of Compliance
- 4.05 | Verification

## Learning Outcome Statements

The candidate should be able to:

- explain why the GIPS standards were created, who can claim compliance, and who benefits from compliance
- describe the key concepts of the GIPS Standards for Firms
- explain the purpose of composites in performance reporting
- describe the fundamentals of compliance, including the recommendations of the GIPS standards with respect to the definition of the firm and the firm’s definition of discretion
- describe the concept of independent verification

## Local Study Notes

### Migrated from `CFA_tier1/Ethical_and_Professional_Standards/M08-GIPS.md`

_Alignment score: 0.39. Original official module field: Introduction to GIPS._

#### M08: 全球投资业绩标准 (Global Investment Performance Standards)

### 🌳 核心知识树

```text
🏆 M04: Introduction to the Global Investment Performance Standards（全球投资业绩标准）
│
├── ⭐ GIPS 的定位 (Purpose of GIPS)
│   ├── 自愿遵守的全球投资业绩展示标准
│   ├── 确保公平展示和充分披露 (fair representation and full disclosure)
│   ├── 增强全球投资业绩报告的可比性和可信度
│   └── 消除业绩展示中的偏差和选择性呈现
│
├── ⭐ 关键概念 — 公司 (Firm)
│   ├── 公司定义必须清晰、可验证
│   ├── 不能随意改变定义来包含或排除业绩
│   └── 一旦宣称合规，必须涵盖全部 discretionary 账户
│
├── ⭐ 关键概念 — 组合组 (Composite)
│   ├── 按相似投资策略分组的账户集合
│   ├── 不能选择性组合业绩 (cherry-picking)
│   └── 每个组合组必须包含所有符合条件的账户
│
├── ⭐ 关键概念 — 自由裁量权 (Discretion)
│   ├── 全权委托 (discretionary) 账户必须包含在组合组中
│   └── 非全权 (non-discretionary) 账户不能包含在合规组合组中
│
├── ⭐ 合规陈述 (Compliant Presentation)
│   ├── 必须包含特定披露和报告要件
│   ├── 包括计算方法说明、费用处理方式、货币单位等
│   └── 📐 GIPS 结构化记忆链: Firm → Discretionary Portfolios → Composites → Presentation
│
├── ⭐ 九大组成部分 (Nine Major Sections)
│   ├── 0: 基本原则 — 合规定义、公司定义
│   ├── 1: 输入数据 — 数据质量和一致性
│   ├── 2: 计算方法 — 收益率的计算标准
│   ├── 3: 组合组构建
│   ├── 4: 披露 — 必须披露和鼓励披露的事项
│   ├── 5: 陈述与报告
│   ├── 6: 房地产 — 单独章节
│   ├── 7: 私募股权 — 单独章节
│   └── 8: 验证 — 第三方评估公司合规流程
│
├── ⭐ 验证 (Verification)
│   ├── 评估公司整体的 GIPS 合规流程 (firm-level)
│   ├── 不保证单项业绩的正确性
│   └── ⚠️ 验证是 process-level assurance，不是数字担保
│
├── 💡 关键洞察
│   ├── GIPS 是自愿标准，但宣称合规后必须全面遵守
│   ├── GIPS 与 Standard III(D) 都关乎诚实的业绩沟通
│   └── 验证是流程评估，不是业绩保证
│
└── ⚠️ 考试陷阱总结
    ├── GIPS 是自愿的，不是强制性的
    ├── 必须全公司合规，不能部分合规
    ├── 合规 vs 验证：验证不保证业绩正确
    ├── 非全权账户不能包含在合规组合组中
    └── GIPS 涵盖股票、债券、房地产、私募股权等
```

## 📖 知识点详解

### 知识点1：GIPS 的定位（Purpose of GIPS）

GIPS 是一套自愿遵守的全球投资业绩展示标准，旨在确保业绩信息的**公平展示和充分披露 (fair representation and full disclosure)**。

**核心理念**：
- 增强全球投资业绩报告的可比性 (comparability) 和可信度 (credibility)
- 消除业绩展示中的偏差和选择性呈现
- 建立行业最佳实践标准

### 知识点2：关键概念（Key Concepts）

**公司 (Firm)**：
- GIPS 合规的公司定义必须是清晰的、可验证的
- 公司界定为投资管理业务单元，不能随意改变定义来包含或排除业绩
- 一旦宣称合规，必须涵盖公司全部 discretionary 账户

**组合组 (Composite)**：
- 按照相似投资策略、目标或限制分组的账户集合
- 不能选择性组合业绩（cherry-picking）
- 每个组合组必须包含所有符合条件的账户

**自由裁量权 (Discretion)**：
- 必须明确区分全权委托 (discretionary) 和限制性非全权 (non-discretionary) 账户
- 全权委托账户必须包含在相应组合组中
- 非全权账户不能包含在 GIPS 合规组合组中

**合规陈述 (Compliant Presentation)**：
- 所有 GIPS 合规的业绩展示必须包含特定披露和报告要件
- 包括计算方法的说明、费用处理方式、货币单位等

### 知识点3：九大组成部分（Nine Major Sections）

| 部分 | 内容 |
|------|------|
| 0 | 基本原则 — 合规定义、公司定义 |
| 1 | 输入数据 (Input Data) — 数据质量和一致性 |
| 2 | 计算方法 (Calculation Methodology) — 收益率的计算标准 |
| 3 | 组合组构建 (Composite Construction) |
| 4 | 披露 (Disclosure) — 必须披露和鼓励披露的事项 |
| 5 | 陈述与报告 (Presentation and Reporting) |
| 6 | 房地产 (Real Estate) — 单独章节 |
| 7 | 私募股权 (Private Equity) — 单独章节 |
| 8 | 验证 (Verification) — 第三方评估公司合规流程 |

### 知识点4：验证（Verification）

**核心区分**：
- 验证 (verification) 评估公司整体的 GIPS 合规流程 (firm-level compliance process)
- 验证不保证单项业绩的正确性 (not a magic performance guarantee)
- 验证是针对流程的保证，不是针对数字的担保

**GIPS and Standard III(D) 的关系**：两者都关乎诚实的业绩沟通 (honest performance communication)，但 GIPS 提供了更具体的展示标准和全球可比性。

### 📐 关键公式表

| 概念 | 含义 | 考试重点 |
|------|------|----------|
| GIPS 合规 | 公司声称其业绩展示符合 GIPS 标准 | 公司定义必须完整，不能选择性合规 |
| 第三方验证 | 独立评估公司合规流程 | 验证不等于业绩保证 |

### 🛠️ 常见考点与解题思路

**考点1：公司的定义**
- **典型场景**：投资公司旗下的一个部门声称 GIPS 合规，但仅包含部分账户
- **解题思路**：GIPS 要求公司定义必须涵盖所有 discretionary 账户，不能选择性合规

**考点2：组合组 vs. 业绩展示**
- **典型场景**：只展示表现最好的账户业绩
- **解题思路**：违反 GIPS 原则，必须按组合组展示，不能 cherry-pick

**考点3：验证的理解**
- **典型场景**：客户询问已验证的 GIPS 报告是否意味着业绩无误
- **解题思路**：验证是对流程的评估，不是对每项业绩数字的保证

### 🚨 易错点与考试陷阱

| ❌ 错误理解 | ✅ 正确理解 |
|-----------|-----------|
| GIPS 是强制性的 | GIPS 是自愿标准，但宣称合规后必须全面遵守 |
| 公司可以只合规一部份 | 必须全公司合规，不能部分合规 |
| verification = 业绩一定没错 | verification 是 process-level assurance |
| 非全权委托账户可以加入组合组 | 非全权账户不能包含在合规组合组中 |
| GIPS 只适用于股票投资 | GIPS 涵盖股票、债券、房地产、私募股权等 |

### 🔄 跨模块关联

- **[[M03-Guidance-for-Standards-I-VII]]** — III(D) Performance Presentation 与 GIPS 的衔接
- **[[M05-Ethics-Application]]** — V(C) 记录保留与 GIPS 记录要求一致
- **[[M03-Guidance-for-Standards-I-VII]]** — 市场诚信与 GIPS 提升业绩可信度的共同目标
## Review Hooks

- Add mistake-driven traps only after they can be traced back to `.system/events/`.
- Keep module naming and order locked to the official 2026 curriculum registry.
- **核心概念串联**：GIPS 的核心是一个结构化逻辑链：公司(Firm)→全权委托账户(Discretionary)→组合组(Composite)→合规陈述(Presentation)。理解这个链条就能把握 GIPS 整体框架。
- **考试策略**：GIPS 题目常考：公司定义(firm definition)的完整性、组合组(composite)的构建规则、合规陈述必须包含的要素、验证(verification)≠业绩保证。重点记忆合规与验证的区别。
- **跨科目接口**：III(D) Performance Presentation 是 GIPS 的伦理基础；业绩展示的诚实性与 FSA M11 的财务分析技术中的比率可比性相关。
