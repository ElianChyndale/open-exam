---
title: "M03 — Standard I: Professionalism"
description: "准则 I：专业性 — 了解法律、独立性与客观性、虚假陈述、不当行为"
module: M03
subject: Ethical_and_Professional_Standards
---

# M03: 准则 I：专业精神 (Standard I: Professionalism)

## 1. 核心知识点 (Core Knowledge Points)

### I(A) 了解法律 (Knowledge of the Law)

**核心义务**：
- 了解与自身职责相关的适用法律 (know applicable law)
- 当法律与 CFA Standards 冲突时，遵循更严格的规则 (follow stricter rule)
- 发现违规行为时必须远离 (dissociate from violations)

**关键场景**：
- 所在国家法律允许但 CFA Standards 禁止 → 遵循 CFA Standards
- 所在国家法律禁止但雇主鼓励 → 必须遵守法律
- 发现同事违规 → 先向 supervisor 或 compliance 报告；若无改善则考虑离职

**【考试陷阱】**：ignorance of law is not a comfortable exam defense。不知道法律不是免责理由。

### I(B) 独立性与客观性 (Independence and Objectivity)

**核心义务**：在提供投资分析、推荐或行动时保持独立判断，不受外部因素影响。

**影响独立性的典型因素**：
- 礼物与款待 (gifts, issuer-paid travel) — 价值微小且公开的礼品可接受
- 雇主压力 (pressure from employer)
- 利益分配中的偏好 (allocation of benefits)
- 投行业务关系对研究部门的影响

**关键原则**：在冲突影响推荐之前保护判断 (protect judgment before conflict becomes recommendation)。

### I(C) 虚假陈述 (Misrepresentation)

**核心义务**：不得做出任何关于投资分析、推荐、资质或服务的虚假或误导性陈述。

**常见形式**：
- **剽窃 (plagiarism)** — 未注明来源使用他人研究成果
- **保证 (guarantees)** — 承诺收益或保证不亏损
- **资质 (credentials)** — 夸大或虚假陈述个人资质
- **模型/来源陈述 (model/source representation)** — 误导性地陈述分析模型或数据来源的准确性

**特别注意**：使用第三方研究时仍需注明来源；口头陈述和书面材料同样适用。

### I(D) 不当行为 (Misconduct)

**核心义务**：不得从事对职业诚信 (professional integrity) 或资本市场声誉产生负面影响的不诚实行为。

**适用范围**：不限于工作中的行为。个人生活中若存在与职业诚信严重不符的行为（如欺诈、盗窃），同样可能违反本条款。

**判断标准**：行为是否"reflects adversely on professional integrity"。

## 2. 关键判断框架 (Key Frameworks)

| 标准 | 核心问题 | 典型考点 |
|------|----------|----------|
| I(A) | 是否了解并遵守了更严格的规则？ | 法律与标准冲突 |
| I(B) | 判断是否受到不当影响？ | 礼品、差旅、压力 |
| I(C) | 陈述是否真实且可验证？ | 剽窃、保证、资质 |
| I(D) | 行为是否损害职业声誉？ | 个人行为合规边界 |

**Stricter Standard Rule**: `required conduct = max(applicable law, CFA Standards)`

## 3. 常见考点与解题思路 (Exam Tips)

### 考点 1：I(A) 了解法律
- 典型场景：新入职发现公司做法违规
- 解题步骤：确认是否真的违规 → 向 supervisor/compliance 报告 → 若未改善则书面记录 → 严重时考虑离职

### 考点 2：I(B) 独立性
- 典型场景：参加发行人付费考察或接受贵重礼品
- 解题思路：任何可能影响判断的礼物/差旅都需要谨慎评估；判断标准是否"reasonable and customary"

### 考点 3：I(C) 虚假陈述
- 典型场景：使用研究报告未注明来源
- 解题思路：即使重新表述了内容，如果没有注明来源，仍然构成剽窃

### 考点 4：I(D) 不当行为
- 典型场景：个人生活中的违法行为是否违反 Standard？
- 解题思路：关键看是否对职业诚信/profession 声誉产生负面影响

## 4. 易错点提醒 (Common Pitfalls)

| 错误理解 | 正确理解 |
|----------|----------|
| 不直接违法就不用研究法律 | 从业者有义务了解适用法律 |
| 只要披露了礼物就不影响独立性 | 某些礼物即使披露也会影响客观判断 |
| 改写他人内容就不算剽窃 | 改写但未注明来源仍构成剽窃 |
| 个人行为与职业无关 | 严重损害职业声誉的个人行为也违规 |

## 5. 跨模块关联 (Cross-Module Links)

- [[M01-Ethics-Trust-and-Professionalism]] — 专业精神与信任的关系
- [[M02-Code-of-Ethics]] — Standard I 对应 Code 中的 Integrity 与 Competence
- [[M04-Standard-II-Capital-Markets-Integrity]] — I(B) 独立性与市场诚信的关联
- [[M05-Standard-III-Duties-to-Clients]] — I(C) 虚假陈述与 III(D) 业绩陈述的边界
