...
│   ├── 合约地图 (Contract Map)
│   │   ├── 标的资产：股票、利率、外汇、信用、大宗商品及其他参照物 (underlying: equity, rate, FX, credit, commodity, other reference) (标的种类)
│   │   ├── 多头 vs 空头；收益取决于标的资产未来状态 (long vs short; payoff depends on future state of underlying) (多空方向)
│   │   └── 结算：实物交割或现金结算 (settlement: physical delivery vs cash settlement) (结算方式)
│   ├── 市场地图 (Market Map)
│   │   ├── 交易所交易：标准化、清算所、保证金 (exchange-traded: standardized, clearinghouse, margin) (交易所)
│   │   ├── 场外交易：定制化、交易对手风险、双边条款 (OTC: customization, counterparty exposure, bilateral terms) (场外市场)
│   │   └── 名义金额放大敞口但不等于在险现金 (notional amount scales exposure but is not automatically cash at risk) (名义金额)
...
│   │   ├── 远期合约：定制化的未来交易义务 (forward: customized future trade obligation) (远期)
│   │   ├── 期货：标准化的每日结算义务 (futures: standardized obligation with daily settlement) (期货)
│   │   └── 互换：一系列类似远期的交换协议 (swap: series of forward-like exchanges) (互换)
│   ├── 或有索取权 (Contingent Claims)
│   │   ├── 看涨期权赋予买入权利；看跌期权赋予卖出权利 (call gives right to buy; put gives right to sell) (看涨看跌)
│   │   ├── 期权持有人拥有权利；卖方承担或有义务 (option holder has right; writer bears contingent obligation) (权利与义务)
│   │   └── 信用衍生品及其他仅在触发时支付 (credit and other claims pay only if trigger/state occurs) (信用衍生品)
...
│   ├── 收益 (Benefits)
│   │   ├── 对冲现有敞口；转换久期、Beta、货币、商品风险 (hedge an existing exposure; transform duration, beta, currency, commodity risk) (风险管理)
│   │   ├── 以资本效率和较低摩擦获得风险敞口 (gain exposure with capital efficiency and lower implementation friction) (高效敞口)
│   │   └── 价格发现与风险转移 (price discovery and risk transfer) (价格发现)
│   ├── 风险与使用者 (Risks and Users)
│   │   ├── 杠杆、流动性、交易对手、基差、模型、操作风险 (leverage, liquidity, counterparty, basis, model, operational risk) (衍生品风险)
│   │   ├── 发行方使用对冲稳定融资/经营现金流 (issuers use hedges to stabilize financing/operating cash flows) (发行方对冲)
│   │   └── 投资者使用衍生品进行风险控制、战术性敞口、套利 (investors use derivatives for risk control, tactical exposure, arbitrage) (投资者用途)
...
│   │   ├── 一价定律适用于相同未来现金流 (law of one price for identical future cash flows) (一价定律)
│   │   ├── 复制组合确定衍生品公平价格 (replication portfolio pins fair derivative price) (复制组合)
│   │   └── 套利行为将错误定价推回公平关系 (arbitrage action pushes mispriced contract back to fair relation) (套利机制)
│   ├── 持有成本关系 (Carry Relation)
│   │   ├── 融资成本提高投资资产的远期价格 (financing cost raises forward price for an investment asset) (融资成本)
│   │   ├── 已知收入/收益率/便利收益减少净持有成本 (known income / yield / convenience benefits reduce net carry) (持有收益)
│   │   └── 存续期价值与初始远期价格不同 (value after initiation differs from original forward price) (存续期价值)
...
│   │   ├── 无收益资产：现货以无风险利率增长 (no-income asset: spot grown at risk-free rate) (无收益)
│   │   ├── 已知收入：扣除收入现值后向前滚动 (known income: subtract PV of income before carrying forward) (已知收入)
│   │   └── 已知收益率：按连续/离散收益率减少持有成本 (known yield: reduce carry by continuous/discrete yield assumption) (已知收益率)
│   ├── 合约价值 (Contract Value)
│   │   ├── 核心公式 (English)
...
│   │   ├── 多头远期在市场远期价格高于合约价格时获利 (long forward gains when market forward price rises above contract price) (多头盈亏)
│   │   ├── 期货逐日盯市每日实现盈亏 (futures marking-to-market realizes gains/losses daily) (每日结算)
│   │   └── 利率相关性可导致期货与远期价格分化 (interest-rate correlation can make futures and forwards diverge) (期现分化)
...
│   │   ├── 固定换浮动利率互换交换净利息现金流 (fixed-for-floating interest rate swap exchanges net interest cash flows) (利率互换)
│   │   ├── 货币互换可能交换本金和票息币种 (currency swap may exchange principal and coupon currencies) (货币互换)
│   │   └── 初始固定利率使起始价值约等于零 (initial fixed rate chosen so inception value is approximately zero) (零值起点)
│   ├── 估值直觉 (Valuation Intuition)
│   │   ├── 互换可视为债券对或远期利率协议组合 (swap can be viewed as bond pair or strip of forward-rate agreements) (估值视角)
│   │   ├── 价值随固定利率与市场互换曲线变化 (value changes as fixed rate vs market swap curve changes) (价值驱动)
│   │   └── 支付方/接收方标签取决于固定端敞口 (payer/receiver label follows fixed leg exposure) (方向判断)
...
│   │   ├── 看涨期权收益 `max(0, S_T - X)`；看跌期权收益 `max(0, X - S_T)` (call payoff; put payoff) (收益公式)
│   │   ├── 内在价值、时间价值、实值状态 (intrinsic value, time value, moneyness) (期权价值)
│   │   └── 收益不包括权利金；利润包括权利金和融资成本 (payoff excludes premium; profit includes premium and financing context) (收益与利润)
│   ├── 平价关系与合成头寸 (Parity and Synthetics)
│   │   ├── 核心公式 (English)
│   │   │   ├── `c + PV(X) = p + S0` 买卖权平价 (put-call parity)
│   │   │   └── `c = p + S0 - PV(X)` 合成看涨期权 (synthetic long call)
│   │   ├── 欧式平价：看涨期权 + 行权价现值 = 看跌期权 + 股票 (European parity: call + PV(strike) = put + stock) (平价公式)
│   │   ├── 移项可合成看涨、看跌、股票、保护性看跌、信托看涨 (rearrange to synthesize call, put, stock, protective put, fiduciary call) (合成头寸)
│   │   └── 上下限揭示套利关系 (lower/upper bounds expose arbitrage relationships) (套利边界)
...
└── M08: 单期二叉树期权估值 (One-Period Option Valuation)【考试核心】↔ 2026 Outline: Binomial Valuation
    ...
    │   ├── 计算上涨/下跌状态的期权收益 (compute option payoff in up/down states) (终端收益)
    │   ├── 对冲比率匹配状态依赖收益差异 (hedge ratio matches state-contingent payoff difference) (对冲比率)
    │   └── 借入/贷出完成复制组合 (borrowing/lending completes replicating portfolio) (借贷复制)
    ├── 风险中性分支 (Risk-Neutral Branch)
    │   ├── 核心公式 (English)
    │   │   ├── `p* = [(1+r)-d]/(u-d)` 风险中性概率 (risk-neutral probability)
    │   │   └── `V0 = [p*Vu + (1-p*)Vd]/(1+r)` 单期期权价值 (one-period option value)
    │   ├── 从无套利增长推导风险中性概率 (derive risk-neutral probability from no-arbitrage growth) (风险中性推导)
    │   ├── 风险中性期望收益以无风险利率贴现 (expected risk-neutral payoff discounted at risk-free rate) (风险中性定价)
    │   └── 更高波动率扩大状态价差并可能提高期权价值 (higher volatility widens state spread and can raise option value) (波动影响)
