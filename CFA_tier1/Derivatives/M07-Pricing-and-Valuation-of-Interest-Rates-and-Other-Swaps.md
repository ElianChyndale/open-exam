...
> **模块定位**：用无套利、复制和工具结构理解远期、期货、互换、期权的风险转移。 本模块聚焦 **Pricing and Valuation of Interest Rates and Other Swaps**，要求把官方 LOS 转成可执行的判断、计算或解释动作。
...
- **Swaps vs. Forwards（核心术语）**：本模块关键词，用于定位 LOS、题干条件和解题动作。
- **Swap Values and Prices（核心术语）**：本模块关键词，用于定位 LOS、题干条件和解题动作。
