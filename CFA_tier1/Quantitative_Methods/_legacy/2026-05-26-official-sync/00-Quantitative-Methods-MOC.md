...
| M08 | M8: Hypothesis Testing | Hypothesis Testing        | 计算+策略 | H0/H1, Type I/II, p-value, pooled/unpooled t, F-test for variances, chi-square test for variance, paired comparisons, parametric vs nonparametric | [[M08-Hypothesis-Testing]] |
...
│   ├── 8.4 Two-Sample Tests (双样本检验)【考试核心】
│       ├── 核心公式 (English)
│       │   ├── `s_p² = [(n₁-1)s₁²+(n₂-1)s₂²]/(n₁+n₂-2)`（合并估计量公式）
│       │   ├── `t = (x̄₁-x̄₂-d₀)/(s_p·√(1/n₁+1/n₂))`（双样本 t 检验，方差相等）
│       │   ├── `t = [(x̄₁-x̄₂)-d₀]/√(s₁²/n₁+s₂²/n₂)`（双样本 t 检验，方差不等）
│       │   ├── `t = d̄/(s_d/√n)`（配对比较 t 检验，df = n-1)
│       │   ├── `F = s₁²/s₂²`（F 检验统计量，比较两总体方差）
│       │   └── `z = (p̂₁-p̂₂)/√[p̂(1-p̂)(1/n₁+1/n₂)]`（双样本比例检验）
│       ├── 8.4.1 Pooled Variance t-Test (合并方差 t 检验)【考试核心】
│       │   ├── 假设：正态分布、方差相等但未知 (Assumes Equal Variances)
│       │   └── df = n₁+n₂-2
│       ├── 8.4.2 Unequal Variance t-Test / Welch (方差不等 t 检验)
│       │   ├── 不合并方差，各自使用样本方差 (Separate Variances)
│       │   ├── `t = [(x̄₁-x̄₂)-d₀]/√(s₁²/n₁+s₂²/n₂)`
│       │   └── 自由度经 Satterthwaite 调整 (Adjusted df)
│       ├── 8.4.3 F-Test for Equality of Variances (方差齐性 F 检验)【考试核心】
│       │   ├── H₀: σ₁² = σ₂² vs H₁: σ₁² ≠ σ₂²
│       │   ├── F 统计量 ~ F(n₁-1, n₂-1)
│       │   └── 对正态性假设极其敏感，偏离正态时不适用 (Highly Sensitive to Non-Normality)
│       ├── 8.4.4 Paired Comparisons Test (配对比较检验)【考试核心】
│       │   ├── 适用：**相关/配对**样本（before/after, matched pairs）
│       │   ├── 本质：对差值 dᵢ = X₁ᵢ - X₂ᵢ 做单样本 t 检验
│       │   ├── df = n-1（n = 配对个数）
│       │   └── 消除了个体间差异，检验力高于独立样本检验
│       └── 8.4.5 Two-Sample Proportion Test (双样本比例检验)
│           └── 合并比例 p̂ = (x₁+x₂)/(n₁+n₂)，使用 z 分布
│   └── 8.5 Chi-Square Test for a Single Variance (单样本卡方方差检验)【考试核心】
│       ├── H₀: σ² = σ₀² vs H₁: σ² ≠ σ₀²（或单边）
│       ├── 核心公式：`χ² = (n-1)s²/σ₀²`，df = n-1
│       ├── 卡方分布非对称右偏，df 增大时趋近正态
│       ├── 对正态性假设极其敏感 (Highly Sensitive to Normality)
│       └── 注意：与 F 检验（两总体方差比较）不同，这是单总体方差检验
│
