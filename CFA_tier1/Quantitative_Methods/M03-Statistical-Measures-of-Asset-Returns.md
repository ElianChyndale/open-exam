...
### Migrated from `CFA_tier1/Quantitative_Methods/M03-Statistical-Measures.md`

_Alignment score: 1.00. Original official module field: Module 3: Statistical Measures of Asset Returns._
...
2. **偏度图坐标轴**：偏度分布图中 y 轴始终表示 frequency（频率），x 轴表示变量取值/收益结果。判断左偏/右偏时先看长尾方向，但读图前先锁定坐标轴含义。
3. **半标准差单位一致性**：下行偏差/目标半标准差（semideviation/target semideviation）的单位必须与原始数据一致——它是下行偏离的平方根，不是平方项。
4. **左偏≠负值多**：左偏分布中大部分数据仍在正值区间，只是左侧长尾有极端负值。
5. **峰度看尾部不看尖峰**：高峰度 = 厚尾 + 更多极端值，不是简单"尖"。
6. **CV 要有意义的前提是均值不为零**：均值接近或等于零时 CV 失去意义。
7. **分位数位置 = (n+1)×p/100**：考试可能让你计算第 k 百分位数的位置和数值。
8. **最大可能中位数用法**：open-ended 分布（无上限或下限）只能用中位数，不能用均值。
