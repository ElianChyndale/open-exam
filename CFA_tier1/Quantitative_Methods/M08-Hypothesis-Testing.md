...
### Migrated from `CFA_tier1/Quantitative_Methods/M08-Hypothesis-Testing.md`

_Alignment score: 1.00. Original official module field: Module 8: Hypothesis Testing._
...
###### 1.4 双样本检验（Two-Sample Tests）

双样本检验是比较两个总体参数是否相等的一类方法。CFA 一级主要考察均值比较和比例比较，并延伸至方差比较。

####### 1.4.1 双样本均值检验 — 方差相等（Pooled Variance t-Test）
...
####### 1.4.2 双样本均值检验 — 方差不相等（Unequal Variance t-Test / Welch's Approximation）
...
####### 1.4.3 双样本方差检验（F-Test for Equality of Two Variances）
...
####### 1.4.4 配对比较检验（Paired Comparisons Test / Dependent Samples t-Test）
...
####### 1.4.5 双样本比例检验（Two-Sample Test of Proportions）
...
###### 1.5 单样本方差检验（Chi-Square Test for a Single Variance）
...
6. **配对比较检验自由度记错**：配对检验本质是对差值做单样本 t 检验，df = n-1（n = 配对个数）。易混淆成独立样本的 df = n₁+n₂-2（即 30 对 → 58），或误记为 n-2。
