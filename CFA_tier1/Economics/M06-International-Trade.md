---
title: "M06 — International Trade"
description: "CFA Level I 2026 official module: International Trade"
module: M06
subject: "Economics"
topic_area: Economics
curriculum_year: 2026
official_module: "Module 6: International Trade"
official_source: CFA Institute Learning Ecosystem scrape, generated 2026-05-25
note_type: official_module_projection
status: active
tags:
  - CFA_L1
  - Economics
  - official_2026
---

# M06: International Trade

> This file is aligned to the CFA Institute 2026 Level I module name and order. Legacy local notes were migrated below when a reliable match was found.

## Official Module Structure

- Learning Outcomes: International Trade
- 6.01 | Introduction
- 6.02 | Benefits and Costs of Trade
- 6.03 | Trade Restrictions and Agreements—Tariffs, Quotas, and Export Subsidies
- 6.04 | Trading Blocs and Regional Integration

## Learning Outcome Statements

The candidate should be able to:

- describe the benefits and costs of international trade
- compare types of trade restrictions, such as tariffs, quotas, and export subsidies, and their economic implications
- explain motivations for and advantages of trading blocs, common markets, and economic unions

## Local Study Notes

### Migrated from `CFA_tier1/Economics/M06-International-Trade.md`

_Alignment score: 1.00. Original official module field: Module 6: International Trade._

#### M06: International Trade（国际贸易）

##### 1. 核心知识点（中英双语讲解）

###### 贸易基础理论（Foundations of Trade）

**绝对优势（Absolute Advantage）**：一个国家用更少的资源生产同一种商品的能力。

**比较优势（Comparative Advantage）**：一个国家生产某种商品的**机会成本（opportunity cost）** 低于其他国家。这是国际贸易的核心理论基础。

即使一国在所有商品生产上都具有绝对劣势，它仍然可以通过专业化生产机会成本最低的商品并进行贸易而获益。

**贸易收益（Gains from Trade）**：专业化生产 → 全球总产出增加 → 贸易各方通过交换分享收益。

###### 贸易限制（Trade Restrictions）

**关税（Tariffs）**：对进口商品征收税收。
- 效果：政府获得收入；国内生产者剩余增加；消费者剩余减少；产生无谓损失
- 净效果：贸易量减少 + 社会福利损失

**配额（Quotas）**：对进口数量设置上限。
- 效果：与关税类似，但配额收益（quota rents）流向进口许可证持有者而非政府
- 通常比关税更扭曲市场

**出口补贴（Export Subsidies）**：政府补贴出口企业。
- 效果：鼓励出口，但扭曲全球贸易格局，损害其他国家生产者

**非关税壁垒（Non-Tariff Barriers）**：包括监管标准、卫生检疫要求、海关程序等，间接增加进口难度。

###### 贸易条件（Terms of Trade）

**贸易条件（Terms of Trade, TOT）** = 出口价格指数 / 进口价格指数
- TOT上升：每单位出口能换取更多进口 → 对该国有利
- TOT下降：出口购买力下降 → 对该国不利

###### 区域经济一体化（Regional Economic Integration）

区域经济一体化从低到高分五个层级：

| 级别 | 名称 | 特征 |
|------|------|------|
| 1 | 自由贸易区（Free Trade Area） | 内部取消关税，各国对非成员自行制定关税 |
| 2 | 关税同盟（Customs Union） | 内部取消关税 + 统一对外关税 |
| 3 | 共同市场（Common Market） | 关税同盟 + 要素自由流动（劳动力、资本） |
| 4 | 经济联盟（Economic Union） | 共同市场 + 协调经济政策 |
| 5 | 货币联盟（Monetary Union） | 经济联盟 + 统一货币和货币政策 |

**常见一体化组织**：
- EU（欧盟）：经济与货币联盟
- USMCA（美墨加协定）：自由贸易区
- ASEAN（东盟）：自由贸易区

**贸易创造（Trade Creation） vs 贸易转移（Trade Diversion）**：
- 贸易创造：从低成本生产国进口取代国内高成本生产 → 提高效率
- 贸易转移：从低成本非成员国进口转向高成本成员国 → 降低效率
- 区域一体化在贸易创造 > 贸易转移时对成员国有利

##### 2. 关键公式（公式+解释+场景）

**机会成本计算**：`Opportunity Cost of Good A = Units of Good B Given Up / Units of Good A Gained`
- 场景：给定两国生产两种商品的产量表，计算各自的机会成本，判断比较优势。

**贸易条件**：`Terms of Trade = Export Price Index / Import Price Index`
- 场景：判断一国贸易条件是否改善。指数上升表示贸易条件改善。

##### 3. 常见考点与解题思路

**考点1：绝对优势 vs 比较优势的判断**
- 绝对优势：看单位投入产出更多（或单位产出需要投入更少）
- 比较优势：看机会成本更低
- 解题关键：先算机会成本，再比较

**考点2：关税/配额的效果分析**
- 关税效果三角形分析：消费者损失 > 生产者收益 + 政府收入 → 净损失
- 配额相比关税：政府失去收入，配额收益归进口商

**考点3：一体化水平的判断**
- 根据特征描述判断属于哪类一体化组织
- 记忆技巧：Free Trade → Customs → Common → Economic → Monetary（从低到高）

##### 4. 易错点提醒

- **贸易政策可以帮助特定群体但仍降低总福利**。例如关税保护国内特定行业就业，但消费者支付更高价格，整体福利下降。
- **比较优势是动态的**：随时间推移，要素积累和技术进步会改变一国的比较优势。
- **配额比关税更扭曲**：配额完全限制数量，市场调节机制更少，通常导致更大的效率损失。
- **自由贸易区vs关税同盟的区别**：自由贸易区需原产地规则（rules of origin）防止贸易偏转（trade deflection）。
- **贸易创造 vs 贸易转移**：区域一体化的净福利效应取决于贸易创造是否超过贸易转移。

##### 5. 跨模块关联

- 贸易净出口是GDP组成部分，见 **[[M03-Fiscal-Policy]]** 中的支出法核算
- 贸易差额对汇率的影响在 **[[M07-Capital-Flows-and-FX-Markets]]** 中讨论
- 关税等贸易政策作为财政政策的一部分见 **[[M03-Fiscal-Policy]]**
- 贸易摩擦和供应链调整可能源于地缘政治风险，见 **[[M05-Introduction-to-Geopolitics]]**
### 🌳 核心知识树

```text
🏆 M06: International Trade（国际贸易）
│
├── ⭐ 贸易理论基础 (Trade Theory) 🎯超高頻
│   ├── 绝对优势 (Absolute Advantage): 更少资源生产相同商品
│   └── 比较优势 (Comparative Advantage): 机会成本更低 🎯超高頻
│       └── 即使所有商品绝对劣势也能从贸易中获益
│
├── ⭐ 贸易收益 (Gains from Trade)
│   ├── 专业化→全球总产出↑
│   ├── 贸易各方通过交换分享收益
│   └── 消费者剩余增加
│
├── ⭐ 贸易限制 (Trade Restrictions) 🎯超高頻
│   ├── 关税 (Tariffs): 进口税收
│   │   ├── 政府收入↑, 国内生产者↑, 消费者↓
│   │   └── 净效果: 无谓损失, 社会福利↓
│   ├── 配额 (Quotas): 进口数量上限
│   │   └── ⚠️ 配额收益归进口商而非政府
│   ├── 出口补贴 (Export Subsidies): 政府补贴出口
│   └── 非关税壁垒: 监管标准、卫生检疫
│
├── ⭐ 贸易条件 (Terms of Trade) 🎯高频
│   ├── 📐 TOT = 出口价格指数 / 进口价格指数
│   ├── TOT↑ → 出口能换更多进口 → 有利
│   └── TOT↓ → 出口购买力下降 → 不利
│
├── ⭐ 区域经济一体化 🎯超高頻
│   ├── 1. 自由贸易区: 内部取消关税
│   │   ⚠️ 需原产地规则防贸易偏转
│   ├── 2. 关税同盟: 统一对外关税
│   ├── 3. 共同市场: 要素自由流动
│   ├── 4. 经济联盟: 协调经济政策
│   └── 5. 货币联盟: 统一货币
│
├── ⭐ 贸易创造 vs 贸易转移 🎯高频
│   ├── 贸易创造: 从低成本国进口取代国内生产 → 效率↑
│   └── 贸易转移: 从低成本非成员转向高成本成员 → 效率↓
│       └── 净福利取决于贸易创造 > 贸易转移
│
└── ⭐ 常见一体化组织
    ├── EU: 经济与货币联盟
    ├── USMCA: 自由贸易区
    └── ASEAN: 自由贸易区
```

## 📖 知识点详解

### 知识点1：贸易基础理论（Foundations of Trade）

**绝对优势（Absolute Advantage）**：一个国家用更少的资源生产同一种商品的能力。

**比较优势（Comparative Advantage）**：一个国家生产某种商品的**机会成本（opportunity cost）** 低于其他国家。这是国际贸易的核心理论基础。

即使一国在所有商品生产上都具有绝对劣势，它仍然可以通过专业化生产机会成本最低的商品并进行贸易而获益。

**贸易收益（Gains from Trade）**：专业化生产 → 全球总产出增加 → 贸易各方通过交换分享收益。

### 知识点2：贸易限制（Trade Restrictions）

**关税（Tariffs）**：对进口商品征收税收。
- 效果：政府获得收入；国内生产者剩余增加；消费者剩余减少；产生无谓损失
- 净效果：贸易量减少 + 社会福利损失

**配额（Quotas）**：对进口数量设置上限。
- 效果：与关税类似，但配额收益（quota rents）流向进口许可证持有者而非政府
- 通常比关税更扭曲市场

**出口补贴（Export Subsidies）**：政府补贴出口企业。
- 效果：鼓励出口，但扭曲全球贸易格局，损害其他国家生产者

**非关税壁垒（Non-Tariff Barriers）**：包括监管标准、卫生检疫要求、海关程序等，间接增加进口难度。

### 知识点3：贸易条件（Terms of Trade）

**贸易条件（Terms of Trade, TOT）** = 出口价格指数 / 进口价格指数
- TOT上升：每单位出口能换取更多进口 → 对该国有利
- TOT下降：出口购买力下降 → 对该国不利

### 知识点4：区域经济一体化（Regional Economic Integration）

区域经济一体化从低到高分五个层级：

| 级别 | 名称 | 特征 |
|------|------|------|
| 1 | 自由贸易区（Free Trade Area） | 内部取消关税，各国对非成员自行制定关税 |
| 2 | 关税同盟（Customs Union） | 内部取消关税 + 统一对外关税 |
| 3 | 共同市场（Common Market） | 关税同盟 + 要素自由流动（劳动力、资本） |
| 4 | 经济联盟（Economic Union） | 共同市场 + 协调经济政策 |
| 5 | 货币联盟（Monetary Union） | 经济联盟 + 统一货币和货币政策 |

**常见一体化组织**：
- EU（欧盟）：经济与货币联盟
- USMCA（美墨加协定）：自由贸易区
- ASEAN（东盟）：自由贸易区

**贸易创造（Trade Creation） vs 贸易转移（Trade Diversion）**：
- 贸易创造：从低成本生产国进口取代国内高成本生产 → 提高效率
- 贸易转移：从低成本非成员国进口转向高成本成员国 → 降低效率
- 区域一体化在贸易创造 > 贸易转移时对成员国有利

### 📐 关键公式表

| 公式 | 解释 | 使用场景 | ⚠️ 注意 |
|------|------|----------|---------|
| 机会成本 = 放弃的B数量 / 获得的A数量 | 生产一单位A的机会成本 | 比较优势计算 | 两个国家分别算 |
| Terms of Trade = Export Price Index / Import Price Index | 贸易条件 | 判断贸易条件变化方向 | ↑=有利, ↓=不利 |
| 关税净福利 = 政府收入 + 生产者剩余变化 + 消费者剩余变化 | 关税的总福利效应 | 分析关税的社会福利影响 | 消费者损失>生产者+政府收益→净损失 |
| 配额租金 = (进口价格 - 国际价格) × 配额数量 | 配额产生的租金 | 比较关税和配额的效果 | 租金归进口商不是政府 |
| 贸易创造收益 = (国内成本 - 伙伴国成本) × 进口量 | 区域一体化效率收益 | 评估区域一体化的收益 | 需比较两种效应 |
| 贸易转移损失 = (伙伴国成本 - 非成员国成本) × 进口量 | 区域一体化效率损失 | 评估区域一体化的成本 | 负面影响 |

### 🛠️ 常见考点与解题思路

**Topic 1: 绝对优势 vs 比较优势判断**
- 绝对优势：看单位投入产出多（或单位产出投入少）
- 比较优势：看机会成本更低
- 解题步骤：① 计算每个国家的机会成本 ② 比较哪个成本更低 ③ 该国在该产品上有比较优势

**Topic 2: 关税/配额效果分析**
- 关税：消费者损失 > 生产者收益 + 政府收入 → 净损失
- 配额：消费者损失 > 生产者收益 + 进口商收益 → 净损失
- 配额比关税更扭曲（市场机制更少）

**Topic 3: 一体化水平判断**
- 从低到高：Free Trade → Customs Union → Common Market → Economic Union → Monetary Union
- 每个层级的核心特征记住

**Topic 4: 贸易创造 vs 贸易转移**
- 贸易创造：从高成本国内转向低成本伙伴 → 效率提升
- 贸易转移：从低成本非成员转向高成本成员 → 效率下降
- 区域一体化净福利取决于哪种效应更大

**Topic 5: 贸易条件变化**
- TOT↑有利（出口更值钱）
- TOT↑原因：出口价格上涨或进口价格下降

### 🚨 易错点与考试陷阱

| ❌ 错误理解 | ✅ 正确理解 | 原因 |
|------------|------------|------|
| 贸易政策只对特定群体有利 | 帮助特定群体（如保护行业就业）但降低总福利 | 消费者支付更高价格 |
| 比较优势是静态的 | 会随时间变化（要素积累和技术进步） | 动态比较优势 |
| 关税比配额更扭曲 | 配额更扭曲，完全限制数量 | 市场调节更少 |
| 自由贸易区=关税同盟 | FTA需原产地规则，关税同盟统一对外关税 | 治理复杂度不同 |
| 贸易创造总是主导 | 需要具体情况具体分析 | 取决于成本和替代关系 |
| 出口补贴对补贴国有利 | 补贴扭曲全球贸易格局 | 损害其他国家生产者 |
| 发展中国家从自由贸易中的收益更大 | 收益分配不均衡 | 但各国都可能获益 |
| 贸易逆差=坏 | 贸易逆差是资本流入 | 经常账户逆差=资本账户顺差 |

### 🔄 跨模块关联

- **贸易收支是GDP组成部分** → [[M03-Fiscal-Policy]]（支出法核算中的净出口）
- **贸易差额与汇率** → M07-Capital-Flows-and-FX-Markets（经常账户→汇率）
- **贸易政策与财政** → [[M03-Fiscal-Policy]]（关税作为财政工具）
- **贸易摩擦与地缘政治** → [[M05-Introduction-to-Geopolitics]]（贸易制裁和关税战）
- **贸易与国际要素流动** → M07-Capital-Flows-and-FX-Markets（资本流动互补）
- **全球供应链与行业分析** → Equity M06（行业分析中的贸易因素）
- **贸易对公司收入的影响** → Equity M07（汇率对跨国公司的收入影响）

### 📋 复习与刷题提示

- **比较优势计算是必考题**：绝对优势vs比较优势的区分判断
- **关税/配额效果分析**：谁受益、谁受损、净福利变化的逻辑链
- **一体化五层次**：从低到高的特征记忆
- **贸易创造vs转移**：概念区分和福利效应的判断
- **贸易条件**：TOT公式和方向判断（↑有利还是不利）
- **补贴效果**：出口补贴对出口国和进口国的影响
- **原产地规则**：自由贸易区和关税同盟的关键区别
- **刷题建议**：mock中比较优势计算最高频，一体化层次判断次之

## Review Hooks

- Add mistake-driven traps only after they can be traced back to `.system/events/`.
- Keep module naming and order locked to the official 2026 curriculum registry.
