...
### Migrated from `CFA_tier1/Financial_Statement_Analysis/M01-Analysis-Framework.md`

_Alignment score: 0.57. Original official module field: M1: Introduction to Financial Statement Analysis._

#### M01: 财务报表分析框架 (Analysis Framework)

##### 1. 核心知识点

###### 1.1 分析框架步骤 (Framework Steps)
...
###### 1.2 信息层级 (Information Hierarchy)
...
| 概念 | 公式/关系 | 说明 |
|------|-----------|------|
| 会计等式 (Accounting Equation) | `Assets = Liabilities + Equity` | 资产负债表的基石 |
| 勾稽关系 (Articulation) | 净利润 -> 留存收益 -> 所有者权益 | 利润表与资产负债表的连接桥梁 |

##### 3. 常见考点与解题思路

- **考点1**：分析框架四个步骤的顺序与内容。解题思路：按"目的→数据→分析→结论"四步记忆，注意每一步骤的输入(input)和输出(output)
- **考点2**：信息源分类。解题思路：区分主要信息源（报表、附注、监管文件）和补充信息源（管理层评论、行业报告、经济数据）
- **考点3**：IFRS vs US GAAP 差异对分析的影响。解题思路：先识别具体项目的准则差异，再评估该差异对财务比率的影响方向

##### 4. 易错点提醒

- **附注是组成部分**: 附注(notes to financial statements)是财务报表的组成部分(integral part)，而非补充材料，其内容包含重大会计政策披露
- **审计意见性质**: 无保留意见(unqualified opinion)不代表公司财务健康——它只表示报表公允呈现，不保证公司没有经营风险
- **动态循环**: 分析框架不是一次性流程，需持续更新(continuous updating)和迭代

##### 5. 跨模块关联

- [[M02-Income-Statement|利润表分析]]：框架第一步中收入确认和费用配比的理解基础
- [[M10-Financial-Analysis-Techniques|财务分析技术]]：框架第三步的数据分析和比率计算在此模块深入展开
- [[M09-Financial-Reporting-Quality|财务报告质量]]：附注质量和报告选择直接影响框架分析的可靠性和结论准确性
## Review Hooks
